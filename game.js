...
document.addEventListener("DOMContentLoaded", () => {
    ...
    /* ================= ASSETS ================= */
    const assets = {};
    const load = (name, src, fallbackColor) => {
        const i = new Image();
        i.src = src;
        assets[name] = { img: i, loaded: false, fallback: fallbackColor };
        i.onload = () => assets[name].loaded = true;
    };
    load("bg", "assets/bg.png", "#1a0b2e"); // Dark Purple BG
    load("wizard", "assets/wizard.png", "blue");
    load("block", "assets/block.png", "#4a4a4a");
    load("spike", "assets/spike.png", "red");
    load("castle", "assets/castle.png", "gold");
    load("dragon", "assets/dragon.png", "purple"); // The Dragon
    load("start_dragon", "assets/start_dragon.png", "purple"); // For Menu/Intro if needed
    /* ================= AUDIO ================= */
    ...
    const startAudio = () => {
        if (!audioStarted) {
            bgm.play().catch(e => console.log("Audio play failed (user interaction needed or missing file)"));
            audioStarted = true;
        }
    };
    ...
    function draw() {
        ...
        ctx.save();
        if (assets.bg.loaded) {
            ctx.drawImage(assets.bg.img, bgX, 0, canvas.width, canvas.height);
            ctx.drawImage(assets.bg.img, bgX + canvas.width, 0, canvas.width, canvas.height);
        } else {
            // Gradient Fallback
        ...
        // World (Blocks)
        blocks.forEach(b => {
            if (assets.block.loaded) ctx.drawImage(assets.block.img, b.x, b.y, b.w, b.h);
            else { ctx.fillStyle = assets.block.fallback; ctx.fillRect(b.x, b.y, b.w, b.h); ctx.strokeStyle = "white"; ctx.strokeRect(b.x, b.y, b.w, b.h); }
        });
        // Spikes
        spikes.forEach(s => {
            if (assets.spike.loaded) ctx.drawImage(assets.spike.img, s.x, s.y, s.w, s.h);
            else {
                ctx.fillStyle = assets.spike.fallback;
        ...
        if (assets.castle.loaded) ctx.drawImage(assets.castle.img, castle.x, castle.y, castle.w, castle.h);
        else { ctx.fillStyle = assets.castle.fallback; ctx.fillRect(castle.x, castle.y, castle.w, castle.h); }
        ctx.restore();
        ...
        if (dragon.x) {
            ctx.save();
            // Hover Effect
            const hoverY = Math.sin(Date.now() / 500) * 5;
            ctx.translate(dragon.x, dragon.y + hoverY);
            const dW = 140; // Slightly larger for better detail
            const dH = 120;
            if (dragon.dir === -1) {
                ctx.translate(dW, 0);
                ctx.scale(-1, 1);
            }
            if (assets.dragon.loaded) {
                // Draw Shadow
                ctx.save();
                ctx.translate(0, dH);
                ctx.scale(1, 0.2);
                ctx.fillStyle = "rgba(0,0,0,0.3)";
                ctx.beginPath(); ctx.arc(dW / 2, 0, 40, 0, Math.PI * 2); ctx.fill();
                ctx.restore();
                ctx.drawImage(assets.dragon.img, 0, 0, dW, dH);
            } else {
                // Placeholder Dragon (Procedural fallback if image missing)
                ctx.fillStyle = assets.dragon.fallback;
                // Simple shape
                ctx.font = "80px Arial";
                ctx.fillText("🐉", 10, 80);
                // Alternatively draw a cool shape
        ...
        const visY = player.y - (player.visualH - player.h);
        ctx.save();
        if (assets.wizard.loaded) {
            if (!player.facingRight) { ctx.translate(visX + player.visualW, visY); ctx.scale(-1, 1); ctx.drawImage(assets.wizard.img, 0, 0, player.visualW, player.visualH); }
            else { ctx.drawImage(assets.wizard.img, visX, visY, player.visualW, player.visualH); }
        } else {
            // Wizard Fallback
            ctx.fillStyle = assets.wizard.fallback;
            ctx.fillRect(visX + 20, visY + 20, player.w, player.h); // Simple rect
            // Hat
            ctx.beginPath(); ctx.moveTo(visX + 20, visY + 20); ctx.lineTo(visX + 60, visY + 20); ctx.lineTo(visX + 40, visY - 20); ctx.fill();
        }
        if (levelWin) {
            ctx.save();
