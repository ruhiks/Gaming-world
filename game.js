...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let hasLava = false; // true for level 4
    const dragonObj = {
        x: 0, y: 0, baseY: 0, w: 120, h: 100,
        dir: -1, attackTimer: 0, active: false,
        fireRate: 120 // how often it fires
    };
    class Fireball {
        constructor(x, y, dir) {
    ...
    /*
      ...
      - Lava floor (entire bottom = instant death)
      - 7 platforms with wide gaps
      - Several moving platforms (fast)
      - Platform-top spikes on some
      - Castle at top-right
      - Dragon fires every 80 frames
    */
    const levels = [
        /* === LEVEL 1 === */
        {
    ...
    function die() {
        ...
        if (lives <= 0) {
            ...
            // Lose a life — respawn at level start
    ...
    function update() {
        ...
        if (hasLava && player.y > canvas.height - 30) die();
        spikes.forEach(s => { if (hit(player, s)) die(); });
    ...
    function draw3DBlock(b) {
        ...
        /* Drop shadow */
        ctx.save();
        ctx.shadowColor = "rgba(0,0,0,0.6)"; ctx.shadowBlur = 14; ctx.shadowOffsetY = 8;
        ...
        bottomGrad.addColorStop(0, "rgba(0,0,0,0.55)");
